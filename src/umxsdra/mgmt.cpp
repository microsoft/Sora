...
void print_status ( )
{
	...
	sprintf ( text, " Data rate: %4dKbps ", gDataRateK );
	clear_printf ("%s\n", text );
    ...
    short dc_re, dc_im;
    dc_re = RxContext.dcEstimated.m128i_i16[0];
    dc_im = RxContext.dcEstimated.m128i_i16[1];
...
void AdjustRxGain11a ( int iG ) {
	int NewGain = RxGain + iG;
	if ( NewGain < MIN_RX_GAIN || NewGain > MAX_RX_GAIN ) return;
	RxGain = NewGain;
...
void UpdateEnergy () {
	energy_max = (energy_max - (energy_max >> 2)) + ((ULONG)(RxContext.__uiSyncMax) >> 2);
