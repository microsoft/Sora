...
void print_status ( )
{
	...
	if ( cfo > 5 || cfo < -5 ) {
        ...
        for (iss = 0; iss < nstream; iss++)
		    SoraURadioSetFreqOffset ( RadioNo[iss], FreqOffset );
	}
	...
	if ( gPHYMode == PHY_802_11N ) {
	    sprintf ( text, " MCS index: %4d             | idle idx %d", gDataRateK, nIdleCnt);
    } else {
	...
	if ( gPHYMode == PHY_802_11N ) {
    	sprintf ( text, " Last received MCS  %4d    ", lastRate );
    } else {
    ...
    if  ( gPHYMode == PHY_802_11A || gPHYMode == PHY_802_11N ) {
		sprintf ( text, " OFDM errors: SyncErr %d InvalidSignal %d CRC32Err %d TDrop %d", 
			err_stat[0], err_stat[1], err_stat[2], err_stat[3]);
		clear_printf ( "%s\n", text );
	}
	...
	clear_printf (" Press 'X', 'x' or Ctrl-C to exit the program\n");
	clear_printf ("+------------------------------------------------------------------------+\n" );	
	clear_printf ("\n" );	
...
BOOLEAN ModulateBuffer11 ( UCHAR* pbuf, int size, LONG needACK )
{
    ...
    if ( gPHYMode == PHY_802_11A ) {
        ...
        hr = ModBuffer11n (pbuf, size, NULL, 0, (ushort)gDataRateK,
            (COMPLEX16**)SampleBuffer, SampleSize, &sample_size );
    }

    hr = TransferBuffers(ptid, SampleBuffer, GetNStream(), sample_size);
	if (SUCCEEDED(hr)) {
...
void ConfigureRxGain ( int RxGain ) {
    ...
    size_t iss;
    for (iss = 0; iss < nstream; iss++) {
	    SoraURadioSetRxPA	(RadioNo[iss], RxPa);  // 0dB
	    SoraURadioSetRxGain (RadioNo[iss], RxG);   // 2G dB
    }
}
...
void UpdateFreqOffset ()
{
	if ( gPHYMode == PHY_802_11A ) {
		...
		PhaseOffset = (PhaseOffset >> 1) + (BB11nDemodCtx.CF_CFOffset::CFO_est() >> 1);
    }
}
...
void UpdateEnergy () {
	if ( gPHYMode == PHY_802_11A ) {
		...
		energy_max = (energy_max - (energy_max >> 2)) + (BB11nDemodCtx.CF_11CCA::cca_pwr_reading() >> 2);
	} else if (gPHYMode == PHY_802_11B){
...
void GainTracking () {
    ...
    // TODO: gain tracking for 802.11n
}
