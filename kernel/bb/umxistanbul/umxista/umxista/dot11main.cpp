...
#define DESC_BR L"Microsoft Network Adapter Multiplexor Driver"
...
BOOLEAN GetMACAddress(MAC_ADDRESS* MACAddress) {
	...
	if (hmod) {
		...
		if (GetAdaptersAddresses) {			
			...
			//
			// If the HWTest driver is bridged, we get the bridge MAC address 
			// instead. This may cause a problem later as the MAC address we
			// get here does not match the true MAC address of HWTest driver
			//
			while(addr) {
				if (wcsncmp(addr->Description, DESC, wcslen(DESC)) && 
					wcsncmp(addr->Description, DESC_BR, wcslen(DESC_BR)) )
				{
					addr = addr->Next;
					continue;
				}
...
void ConfigureRadio () 
{
	if ( bWbx ) {
		// for WBX only
		SoraURadioSetCentralFreq (TARGET_RADIO, gChannelFreq );
	} else {
		SoraURadioSetCentralFreq (TARGET_RADIO, gChannelFreq*1000 );
	}
	// Set Tx Gain to a fixed value
	SoraURadioSetTxGain ( TARGET_RADIO, TxGain ); // 21 dBm
	...
	while(1)
		if ( _kbhit () ) {
			...
			case ';':
				...
				break;
			case 'i':
				IncreaseCCAThreshold ();
				break;
			case 'k':
				DecreaseCCAThreshold ();
				break;
...
bool StartBaseband11a () {
    ...
    if (gSampleRate == 20) {
        // TX
	    pBB11aTxSource = CreateModGraph11a_20M ();
        pBB11aPreambleSource = CreatePreamble11a_20M ();
        // RX
        CreateDemodGraph11a_20M(pBB11aRxSource, pBB11aViterbi, pBB11aCarrierSense);
    }
    else if ( gSampleRate == 10 ) {
        // TX
        pBB11aTxSource = CreateModGraph11a_10M ();
        pBB11aPreambleSource = CreatePreamble11a_10M ();
        // RX
        CreateDemodGraph11a_10M(pBB11aRxSource, pBB11aViterbi, pBB11aCarrierSense);
    } else {
		// TX
        pBB11aTxSource = CreateModGraph11a_5M ();
        pBB11aPreambleSource = CreatePreamble11a_5M ();
        // RX
        CreateDemodGraph11a_5M(pBB11aRxSource, pBB11aViterbi, pBB11aCarrierSense);
	}
	...
	hTxThread_11a = _beginthread ( Dot11aSendThread, 0, NULL);
	if (hTxThread_11a == NULL ) {
...
void StopBaseband () {
	...
	IReferenceCounting::Release (pBB11aCarrierSense);
}
