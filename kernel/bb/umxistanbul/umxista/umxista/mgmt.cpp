...
// rate control
ULONG                nDataPacketDrop    = 0;
ULONG                nDataPacketSend    = 0;
ULONG                nDataPacketRetry   = 0;
int					 retry_per_pkt = 0;
...
// ULONG               RxGain = 44; 
ULONG               RxPa   = 0x3000; // 2450 specific
ULONG               RxG    = 0x600;  // 2450 specific

ULONG				TxGain = 0x100; // started low
...
#ifdef _BB20M_
	cfo	= 1.0 * PhaseOffset * 20000 / 0x7FFF / 2 / 3.141593;
#else
	cfo	= 1.0 * PhaseOffset * 10000 / 0x7FFF / 2 / 3.141593;
#endif
...
//		SoraURadioSetFreqOffset ( TARGET_RADIO, FreqOffset );
	...
	sprintf_s ( text, 256, " Operation mode: %20s | Channel freq %dMHz at radio %d", opModeMsg[OpMode], gChannelFreq, TARGET_RADIO );
	clear_printf ( "%s\n", text );

	sprintf_s ( text, 256, " Association State: %s", assoMsg[assoState] );
	clear_printf ( "%s\n", text );

	if ( OpMode == CLIENT_MODE ) {	
		sprintf_s ( text, 256, " SSID: %20s  | Bssid: %02X-%02X-%02X-%02X-%02X-%02X", 
						ssid,
	...
	// mac
	sprintf_s ( text, 256, " Local MAC address: %02X-%02X-%02X-%02X-%02X-%02X  | MAC Queue %u (%u)", 
...
//	sprintf ( text, " RxGain: %2ddB (%4x, %4x)   | Energy max %10d   | Auto %d", RxGain, RxPa, RxG, energy_max, fAutoCfg);
	sprintf_s ( text, 256, " Rx PA:%4x; Rx G %4x; Tx G %4x | CCA Th %d | Auto %d", RxPa, RxG, TxGain, gCCAThreshold, fAutoCfg);
	clear_printf ("%s\n", text );

	sprintf_s ( text, 256, " Gain ctrl state %d | Energy max %10d | goodBeaconCnt %d  ", rxgState, energy_max, goodBeaconCnt);
	clear_printf ("%s\n", text );

	sprintf_s ( text, 256, " Data rate: %4dKbps (auto %d - %d)  | idle idx %d", gDataRateK, fAutoRate, retry_per_pkt, nIdleCnt);
	clear_printf ("%s\n", text );
	nIdleCnt = 0;

	sprintf_s ( text, 256, " CFO to AP %+.2lfKHz          | Freq Offset setting %d  ", cfo, FreqOffset );
	clear_printf ("%s\n", text );

	sprintf_s ( text, 256, " Frame received %6d | Frame sent %6d | Beacon received %6d ", 
			nDataPacketRcvCnt, nDataPacketSndCnt, nBeaconRcvCnt );
	clear_printf ( "%s\n", text );

	sprintf_s ( text, 256, " Interference %6d   | Lost pkt %6d   | Ack received %6d ", nInterferenceCnt, nDataPacketDropCnt, nAckRcvCnt );
	clear_printf ( "%s\n", text );

	sprintf_s ( text, 256, " Last MAC Src %02X-%02X-%02X-%02X-%02X-%02X Dst %02X-%02X-%02X-%02X-%02X-%02X", 
	...
	sprintf_s ( text, 256,  " Last MAC Bssid %02X-%02X-%02X-%02X-%02X-%02X", 
	...
	sprintf_s ( text, 256,  " Last received rate %4dKbps", lastRate );
	clear_printf ( "%s\n", text );
	...
	sprintf_s ( text, 256, " DC <%+06d,%+06d>", dc_re, dc_im);
	clear_printf ( "%s\n", text );

    if  ( gPHYMode == PHY_802_11A ) {
		sprintf_s ( text, 256, " OFDM errors: sync error %d invalid signal field %d crc32 error %d ", 
	...
	clear_printf (" Press '<' '>' - adjust the rx PA\n");
...
BOOLEAN ModulateBuffer11 ( UCHAR* pbuf, int size, LONG needACK )
{
    ...
    if ( gPHYMode == PHY_802_11A ) {
	    hr = ModBuffer11a (pbuf, size, NULL, 0, 
							// (ushort)gDataRateK, 
							6000, // always use the lowest rate for mgmt packets
...
void SetRadioRxGain () {
	SoraURadioSetRxPA	(TARGET_RADIO, RxPa);  
...
void IncreaseCCAThreshold () {
	gCCAThreshold = gCCAThreshold * 126 / 100; 
}

void DecreaseCCAThreshold () {
	gCCAThreshold = gCCAThreshold * 100 / 126;
	if ( gCCAThreshold < MIN_CCA_TH ) 
		gCCAThreshold = MIN_CCA_TH;
}

//
// Increase Rx Gain by one level
//
// Here we assume only OFDM
bool IncreaseGain () {
	RxG += 0x100; 
	if ( RxG > MAX_RX_GAIN ) RxG = MAX_RX_GAIN;
...
void IncreaseTxGain () {
	if (TxGain < 0x1f00 ) {
		TxGain += 0x100;
		SoraURadioSetTxGain (TARGET_RADIO, TxGain);
	}
}
...
static int ar_inc_damp = 10;

void AutoRate () {
	int nTotal = nDataPacketDrop + nDataPacketSend;

	if ( nTotal > 50 ) {
		nDataPacketRetry += nDataPacketDrop * PACKET_MAX_RETRY;

		PlotText ( "autorate", "retry %d drop %d send %d damp %d", nDataPacketRetry, nDataPacketDrop, nDataPacketSend, ar_inc_damp );

		// move rate up / down
		retry_per_pkt = 10*nDataPacketRetry / nTotal;
		if ( retry_per_pkt < 12 ) {
			if ( ar_inc_damp > 0 ) ar_inc_damp --;
			if ( ar_inc_damp == 0 ) {
				IncreaseRate ();
				ar_inc_damp = 10;
			}
		} else if ( retry_per_pkt > 15 ) {
			DecreaseRate ();
			ar_inc_damp = 10;
		} else {
			if ( ar_inc_damp < 10 ) ar_inc_damp++;
		}

		
		nDataPacketDrop = nDataPacketRetry = nDataPacketSend = 0;
	}
...
void misc_mgmt_process () {
	// print out Rx DC
	PlotDots ( "rx dc", (COMPLEX16*)&BB11aDemodCtx.CF_VecDC::direct_current(),  1 );
}
...
void ProcessBeacon (PDOT11_MAC_BEACON beacon) 
{
	...
	if ( OpMode == CLIENT_MODE ) {
			 ...
			 memcmp (beacon->SSID.SSID, adhoc_ssid, adhoc_ssid_len ) == 0 )
		{
			...
			PlotDots ( "Adhoc_beacon", BB11aDemodCtx.CF_Constellation::constellation(), 64 );
