...
uchar FrameBuffer [1024*1024];
ULONG FrameBufSize = 1024*1024;

void usage () {
	printf ( "ofdmRx cmd parameter \n" );
	printf ( "--------------------------------------------------\n" );
...
//		hr = SoraUEnableGetTxPacket();
		...
		if ( ofdmRx.Create ( &RxStream, FrameBuffer, FrameBufSize) ) {
			ofdmRx.Start ();
			printf ("OFDM Rx started...\n" );
			printf ("Trying to demodulate and decode any data packet over wireless..\n" );
		}
